import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ContainerBuilder, Guild, MessageFlags, TextDisplayBuilder } from "discord.js";
...
function buildDashboard(): ContainerBuilder {
  return new ContainerBuilder().setAccentColor(0x1f4f78)
    .addMediaGalleryComponents((gallery) => gallery.addItems((item) => item.setURL("attachment://dashboard.png")))
...
export async function createAtlasDashboard(guild: Guild, channelId: string): Promise<string> {
  ...
  const payload = { content: null, embeds: [], components: [buildDashboard()], files: [{ attachment: BANNER_PATH, name: "dashboard.png" }], flags: MessageFlags.IsComponentsV2 };
  if (config.dashboardMessageId) {
    const existing = await channel.messages.fetch(config.dashboardMessageId).catch(() => null);
    if (existing) { await existing.edit(payload); await prisma.guildConfig.update({ where: { requestGuildId: guild.id }, data: { dashboardChannelId: channel.id } }); return existing.id; }
  }
