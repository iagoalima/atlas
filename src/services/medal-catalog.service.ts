...
function buildDeliveryPermissionRoles(guild: Guild, roles: { roleId: string }[]): string {
  if (roles.length === 0) return "_Nenhum cargo autorizado para entrega configurado._";

  return roles.map(({ roleId }) => {
    const role = guild.roles.cache.get(roleId);
    if (!role) return `\`Cargo não encontrado: ${roleId}\``;
    return `<@&${role.id}>`;
  }).join(" • ");
}
...
export async function buildCatalogPresentation(guild: Guild): Promise<ContainerBuilder> {
  const [categoryCount, medalCount] = await Promise.all([
    prisma.medalCategory.count({ where: { active: true } }),
    prisma.medal.count({ where: { active: true, category: { active: true } } }),
  ]);

  const container = new ContainerBuilder().setAccentColor(0x1f4f78);

  container.addMediaGalleryComponents((gallery) =>
    gallery.addItems((item) => item.setURL("attachment://catalogo.png"))
  );
...
async function upsertCatalogPresentation(guild: Guild, channel: TextChannel): Promise<string> {
  ...
  const payload = {
    ...
    files: [{ attachment: BANNER_PATH, name: "catalogo.png" }],
  ...
  await prisma.guildConfig.update({
    ...
    data: { medalCatalogMessageId: message.id },
  });
  return message.id;
}
...
export async function buildMedalCategoryComponents(guild: Guild, categoryId: string): Promise<ContainerBuilder | null> {
  const category = await prisma.medalCategory.findUnique({
    ...
    include: {
      medals: {
        where: { active: true },
        include: { deliveryPermissionRoles: { select: { roleId: true } } },
        orderBy: { name: "asc" },
      },
    },
  });
  ...
  container.addTextDisplayComponents(
    new TextDisplayBuilder().setContent([
      ...
      "",
      category.description ? cleanText(category.description) : "### Sistema Oficial de Condecorações",
    ].join("\n"))
  );
  ...
  for (let index = 0; index < category.medals.length; index++) {
    ...
    sections.push("", "**Requisitos**", cleanText(medal.requirements));

    if (medal.jurisprudence) {
      sections.push("", "**Jurisprudência**", cleanText(medal.jurisprudence));
    }

    sections.push(
      "",
      "**Autorização**",
      buildDeliveryPermissionRoles(guild, medal.deliveryPermissionRoles)
    );
...
export async function createMedalCategoryCatalog(guild: Guild, categoryId: string): Promise<string | null> {
  ...
  const channel = guild.channels.cache.get(config.medalCatalogChannelId);
  ...
  await prisma.medalCategory.update({ where: { id: category.id }, data: { catalogMessageId: message.id } });
  return message.id;
}
...
export async function updateMedalCategoryCatalog(guild: Guild, categoryId: string): Promise<boolean> {
  ...
  if (!container) {
    if (category.catalogMessageId) {
      try {
        const oldMessage = await channel.messages.fetch(category.catalogMessageId);
  ...
  try {
    const message = await channel.messages.fetch(category.catalogMessageId);
    ...
    try {
      ...
      console.error("❌ [CATALOG] Erro ao recriar mensagem da categoria:", sendError);
      return false;
    }
  }
}
...
export async function syncMedalCatalog(guild: Guild): Promise<boolean> {
  ...
  for (const category of categories) {
    if (category.active || !category.catalogMessageId) continue;
...
export async function createMedalCatalog(guild: Guild, channelId: string): Promise<string | null> {
  await prisma.guildConfig.updateMany({
    where: { requestGuildId: guild.id },
    data: { medalCatalogChannelId: channelId },
  });
...
export async function updateMedalCatalog(guild: Guild): Promise<boolean> {
