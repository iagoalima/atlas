...
import { renderSeasonalReviewPanel } from "./request-review-panel.service.js";
...
export async function handleSeasonalReviewButton(interaction: ButtonInteraction): Promise<boolean> {
  ...
  if (action === "approve" || action === "deny") {
    ...
    if (tm.status !== "PENDING") {
      await interaction.reply({ content: "⚠️ Esta medalha já foi analisada.", flags: MessageFlags.Ephemeral });
      return true;
    }

    if (action === "deny") {
      const modal = new ModalBuilder()
        .setCustomId(`request_deny:${tm.id}`)
        .setTitle("Negar solicitação");

      const reason = new TextInputBuilder()
        .setCustomId("reason")
        .setLabel("Justificativa")
        .setPlaceholder("Explique o motivo da negativa")
    ...
    const updated = await prisma.ticketMedal.update({
      where: { id: tm.id },
    ...
    await logAuditEvent({
      ...
      targetId: updated.ticket.userId,
      ticketId: updated.ticketId,
      medalId: updated.medalId,
      details: { ticketMedalId: updated.id, medalName: updated.medal.name, status: "APPROVED" },
    });

    await renderSeasonalReviewPanel(interaction.message, updated.ticketId);
    await interaction.reply({ content: "## 🟢 Medalha aprovada\n\nA decisão foi registrada.", flags: MessageFlags.Ephemeral });
    return true;
  }

  if (tm.status !== "APPROVED") {
    await interaction.reply({ content: "## ⚠️ Entrega indisponível\n\nEsta medalha precisa estar aprovada antes da entrega.", flags: MessageFlags.Ephemeral });
    return true;
  }

  if (!tm.medal.deliveryPermissionRoles.some((r) => member.roles.cache.has(r.roleId))) {
    await interaction.reply({ content: "## 🔒 Permissão insuficiente\n\nVocê não possui autorização para entregar esta medalha.", flags: MessageFlags.Ephemeral });
    return true;
  }
  ...
  try {
    const result = await deliverMedal({
    ...
    await renderSeasonalReviewPanel(interaction.message, tm.ticketId);
...
export async function handleSeasonalReviewModal(interaction: ModalSubmitInteraction): Promise<boolean> {
  ...
  const tm = await prisma.ticketMedal.findUnique({
    where: { id },
    include: { ticket: true, medal: { include: { approvalRoles: true } } },
  });
  ...
  if (!config?.staffRoleId || !member.roles.cache.has(config.staffRoleId) || !tm.medal.approvalRoles.some((r) => member.roles.cache.has(r.roleId))) {
    await interaction.reply({ content: "## 🔒 Permissão insuficiente\n\nVocê não pode negar esta medalha.", flags: MessageFlags.Ephemeral });
    return true;
  }
  ...
  const updated = await prisma.ticketMedal.update({
    where: { id },
    data: { status: "DENIED", decidedBy: interaction.user.id, reason },
    include: { medal: true, ticket: true },
  });
  ...
  await logAuditEvent({
    ...
    ticketId: updated.ticketId,
    medalId: updated.medalId,
    details: { ticketMedalId: updated.id, medalName: updated.medal.name, status: "DENIED", reason },
  });

  const channel = interaction.channel;
  if (channel?.isTextBased() && "messages" in channel) {
    const panel = await channel.messages.fetch({ limit: 20 }).then((messages) =>
      messages.find((message) => message.author.id === interaction.client.user?.id && message.components.length > 0)
    ).catch(() => null);
    if (panel) await renderSeasonalReviewPanel(panel, updated.ticketId);
  }
