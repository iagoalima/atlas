...
const BANNER_PATH = path.resolve(
  process.cwd(),
  "assets/panels/dashboard.png"
);

export async function buildAnalysisDashboard(
  guild: Guild
): Promise<ContainerBuilder> {
  const [pending, approved, denied, granted, tickets] = await Promise.all([
    prisma.ticketMedal.count({
      where: {
        status: "PENDING",
        ticket: { requestGuildId: guild.id },
      },
    }),
    prisma.ticketMedal.count({
      where: {
  ...
  const container = new ContainerBuilder().setAccentColor(
    pending > 0 ? 0xf1c40f : 0x2ecc71
  );

  container.addMediaGalleryComponents((gallery) =>
    gallery.addItems((item) =>
      item.setURL("attachment://dashboard.png")
    )
  );
  ...
  container.addTextDisplayComponents(
    new TextDisplayBuilder().setContent(
      "## 📂 Solicitações prontas para análise"
    )
  );
  ...
  for (const ticket of tickets) {
    const pendingMedals = ticket.medals.filter(
      (medal) => medal.status === "PENDING"
    ).length;
    const approvedMedals = ticket.medals.filter(
      (medal) => medal.status === "APPROVED"
    ).length;
    ...
    container.addActionRowComponents(
      new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder()
          .setCustomId(`atlas_analysis_open:${ticket.id}`)
          .setLabel("Abrir análise")
          .setEmoji("🔎")
          .setStyle(ButtonStyle.Primary),
        new ButtonBuilder()
          .setStyle(ButtonStyle.Link)
          .setLabel("Abrir solicitação")
          .setURL(
  ...
  container.addTextDisplayComponents(
    new TextDisplayBuilder().setContent(
      "-# Atlas • Central Interna de Análise • Atualize o painel para consultar os dados mais recentes."
    )
  );
...
export async function publishAnalysisDashboard(
  guild: Guild,
  channelId: string
): Promise<string> {
  ...
  if (
    ...
    throw new Error(
      "Canal de análise não encontrado ou sem permissão para envio."
    );
  }
