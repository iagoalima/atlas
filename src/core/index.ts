...
client.on("messageCreate", async (message) => {
  ...
  } catch (error) { console.error("❌ [MESSAGE] Erro ao processar mensagem:", error); }
});
...
client.on("interactionCreate", async (interaction) => {
  try {
    ...
    if (interaction.isStringSelectMenu()) {
      ...
      if (interaction.customId.startsWith("medal_category_select:")) { const { handleMedalCategorySelect } = await import("../commands/medal.js"); await handleMedalCategorySelect(interaction); return; }
      if (interaction.customId.startsWith("medal_delivery_roles:")) { const { handleMedalDeliveryRoleSelect } = await import("../commands/medal.js"); await handleMedalDeliveryRoleSelect(interaction); return; }
      if (interaction.customId.startsWith("medal_approval_roles:")) { const { handleMedalApprovalRoleSelect } = await import("../commands/medal.js"); await handleMedalApprovalRoleSelect(interaction); return; }
      if (interaction.customId.startsWith("medal_delivery_permission_roles:")) { const { handleMedalDeliveryPermissionRoleSelect } = await import("../commands/medal.js"); await handleMedalDeliveryPermissionRoleSelect(interaction); return; }
      return;
    }
    if (interaction.isModalSubmit()) {
      ...
      if (interaction.customId === "setup_delivery_guild_modal") { const { handleSetupDeliveryGuildModal } = await import("../interactions/buttons/setup.buttons.js"); await handleSetupDeliveryGuildModal(interaction); return; }
      return;
    }
