...
import { refreshAnalysisDashboard } from "../services/analysis-dashboard.service.js";
  ...
  try {
    ...
    if (handledRequest) {
      ...
      if (ticket?.requestGuildId) {
        ...
        const requestGuild = await message.client.guilds.fetch(ticket.requestGuildId).catch(() => null);
        if (requestGuild) await refreshAnalysisDashboard(requestGuild);
      }
      return;
    }
...
client.on("interactionCreate", async (interaction) => {
  try {
    ...
    if (interaction.isButton()) {
      ...
      if (interaction.customId.startsWith("atlas_dashboard_catalog")) {
        const config = interaction.guild
          ? await (await import("../infrastructure/database/prisma.js")).prisma.guildConfig.findUnique({
              where: { requestGuildId: interaction.guild.id },
              select: { medalCatalogChannelId: true },
            })
          : null;

        if (!config?.medalCatalogChannelId) {
          await interaction.reply({ content: "❌ O catálogo ainda não foi configurado.", flags: 64 });
          return;
        }
      ...
      if (interaction.customId.startsWith("atlas_analysis_open:")) {
        ...
        const ticket = ticketId
          ? await (await import("../infrastructure/database/prisma.js")).prisma.ticket.findUnique({ where: { id: ticketId } })
          : null;

        if (!ticket) {
          await interaction.reply({ content: "❌ Solicitação não encontrada.", flags: 64 });
          return;
        }

        await interaction.reply({
          content: `🔎 **Solicitação #${ticket.ticketNumber}**\n\nA análise individual está em ${interaction.guild?.channels.cache.get(ticket.channelId) ?? `\`${ticket.channelId}\``}.`,
          flags: 64,
        });
        return;
      }
      ...
      if (
        interaction.customId.startsWith("ticket_medal_approve:") ||
        interaction.customId.startsWith("ticket_medal_deny:") ||
        interaction.customId.startsWith("ticket_medal_deliver:")
      ) {
    ...
    try {
      ...
      if ((replyError as { code?: number }).code === 10062) {
        console.warn("⚠️ [INTERACTION] A interação já havia expirado antes da resposta de erro.");
      } else {
    ...
    if (interaction.guild) {
      await refreshAnalysisDashboard(interaction.guild);
    }
  }
});
...
startRequestNotifications(client);
